...
[![CI](https://github.com/haxlys/genai-lab/actions/workflows/ci.yml/badge.svg)](https://github.com/haxlys/genai-lab/actions/workflows/ci.yml)

Microsoft 공식 *Generative AI for Beginners* (22개 레슨) 코스를 한국어로 따라가며,
변수를 직접 조정해 LLM 동작을 체감할 수 있는 학습 실험실.

> **라이브 데모**: 배포 후 추가 예정 (Vercel/Netlify에서 main 자동 배포)

## 화면

| 화면 | 설명 |
|---|---|
| 랜딩 | 4주 학습 경로 카드 + 22개 레슨 그리드 |
| 레슨 (split-pane) | 좌측 한국어 본문 + 우측 인터랙티브 변수 패널 |
| Run 비교 | side-by-side로 두 Run의 입력/출력 diff 강조 |
| 이미지 생성 (09) | DALL-E 결과 그리드 + revised prompt |
| Agent (17) | step-by-step tool call trace |

> 스크린샷은 추후 `docs/screenshots/`에 추가 예정. 직접 보려면 `pnpm dev` 실행.
...
### GitHub Models PAT 발급 (무료)

1. https://github.com/settings/tokens?type=beta → **Generate new token (fine-grained)**
2. **Token name**: `genai-lab` (자유)
3. **Repository access**: `Public repositories (read-only)`
4. **Account permissions** → **Models** → `Read-only`로 설정
5. **Generate token** 후 `github_pat_...` 토큰을 복사
6. `.env`에 `GITHUB_TOKEN=github_pat_...`로 저장 또는 Settings 페이지에서 입력

권장 모델: `gpt-4o-mini` (빠르고 저렴) → `gpt-4o` (강력함) → `Llama-3.1-70B` (긴 답변).

### 첫 Run 시도

1. `/lessons` → 06번 "텍스트 생성 앱 만들기" 클릭
2. 우측 패널의 프리셋 중 하나를 클릭
3. **Run** 버튼 → 5초 내 한국어 응답 스트리밍

설정 변경 후 다시 Run하면 결과가 자동으로 history에 저장됩니다 (`/history`에서 비교 가능).
...
새 lesson-spec, 버그 수정, UX 개선 모두 환영합니다. 가이드는 [CONTRIBUTING.md](CONTRIBUTING.md)에서 확인하세요.
